...
import RetailerPurchases from './RetailerPurchases';
import RetailerWarehouse from './RetailerWarehouse';
...
const Retailer = () => {
  ...
  // Fetch retailer's products and orders
  const fetchData = async () => {
    try {
      const [productsRes, ordersRes] = await Promise.all([
        axios.get('/retailers/products'),
        axios.get('/retailers/orders')
      ]);
  ...
  const handleDeleteProduct = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        await axios.delete(`/retailers/products/${id}`);
  ...
  const renderContent = () => {
    if (showAddProduct) {
      return (
        <AddProduct
          ...
          submitEndpoint="/retailers/products"
    ...
    switch (activeTab) {
      ...
      case 'cart':
        return <RetailerCart onCheckoutSuccess={() => setActiveTab('purchases')} />;


      case 'questions':
        return <RetailerQuestions />;

      case 'purchases':
        return <RetailerPurchases />;

      case 'warehouse':
        return <RetailerWarehouse />;
        ...
        <div className="w-64 min-h-screen bg-gray-800 border-r border-gray-700">
          ...
          <nav className="mt-6 px-4 space-y-2">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'wholesale'
                ? 'bg-blue-600 text-white'
                : 'text-gray-400 hover:bg-gray-700 hover:text-white'
                }`}
            >
              <span>🏪</span> Wholesale Market
            </button>
            <button
              onClick={() => {
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'warehouse'
                ? 'bg-blue-600 text-white'
                : 'text-gray-400 hover:bg-gray-700 hover:text-white'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'questions'
                ? 'bg-blue-600 text-white'
                : 'text-gray-400 hover:bg-gray-700 hover:text-white'
                }`}
            >
              <span>💬</span> Customer Query
            </button>
            <button
              onClick={() => {
                setActiveTab('cart');
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'cart'
                ? 'bg-blue-600 text-white'
                : 'text-gray-400 hover:bg-gray-700 hover:text-white'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'purchases'
                ? 'bg-blue-600 text-white'
                : 'text-gray-400 hover:bg-gray-700 hover:text-white'
                }`}
            >
